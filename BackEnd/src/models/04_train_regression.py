...
from src.utils.modeling import build_preprocessor, build_temporal_splits, extract_feature_importance, regression_metrics
...
TARGET_COLUMN = "target_casos_t_plus_1"
N_ESTIMATORS = 120
MAX_DEPTH = 18
MIN_SAMPLES_LEAF = 3
...
def main() -> None:
    ...
    validation_metrics = regression_metrics(val_df[TARGET_COLUMN], val_predictions)
    ...
    test_metrics = regression_metrics(test_df[TARGET_COLUMN], test_predictions)

    prediction_df = test_df[
        ["cod_canton", "canton", "provincia", "anio", "semana", "period_id", TARGET_COLUMN]
    ].copy()
    prediction_df["prediccion_regresion"] = test_predictions

    feature_importance_df = extract_feature_importance(final_pipeline)

    save_dataframe(prediction_df, REPORTS_DIR / "regression_test_predictions.csv")
    save_dataframe(feature_importance_df, REPORTS_DIR / "regression_feature_importance.csv")
    save_json(
        {
        ...
        REPORTS_DIR / "regression_metrics.json",
    )
    ...
    joblib.dump(
        ...
        MODELS_DIR / "regression_random_forest.joblib",
    )
    print(f"Modelo de regresion guardado en: {MODELS_DIR / 'regression_random_forest.joblib'}")
